#!/usr/bin/env bash
# Setup noeud APP (web + analyzer + 1 worker) - ex: node7.lan
# Usage:
...
echo "==> Setup APP node StreamNews -> $APP_DIR (data=$DATA_HOST)"
...
echo "==> APP node pret. UI: http://${LOCAL_IP}:3000"
echo "    Workers supplementaires: sudo DATA_HOST=$DATA_HOST bash deploy/setup-worker-node.sh"
